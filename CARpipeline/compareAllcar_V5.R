...
enriched_file <- as.character(args[2])
pacbio_file <- as.character(args[3])
scRNA_file <- as.character(args[4])
out_prefix <- as.character(args[5])

## setwd("Z:/R_Projects/pacbio/comparaMiseqPacbio")
## miseq_file <- "S925_comparation/S925_miseq_final.tsv"
## enriched_file <- "S925_comparation/S925_enriched_final.tsv"
## pacbio_file <- "S925_comparation/S925_pacb2_final.tsv"
## scRNA_file <- "S925_comparation/S925.rna_submtx.tsv"
## out_prefix <- "S925_uniq_v5/S925"


miseq = read.delim(miseq_file,sep = "\t",header = T)
enriched = read.delim(enriched_file,sep = "\t",header = T)
...
enriched$car <- gsub("pMH.*_(LH.*)", "\\1", enriched$car)
enriched$car <- gsub("_BBz", "", enriched$car)
enriched$car <- gsub("_B7H3", "", enriched$car)
enriched$car <- gsub("_HL", "-HL", enriched$car)
enriched$car <- gsub("_LH", "-LH", enriched$car)

#the intersect should be 14
print(length(intersect(intersect(miseq$car,enriched$car),pacbio$car)))

####################################################
##filter enriched data supported with only 1 and 2 read exist
print(nrow(enriched[which(enriched$read_count==1),]))
print(nrow(enriched[which(enriched$read_count>=3),]))

enriched <- enriched[which(enriched$read_count>=3),]
...
## unique_bc <- unique(c(miseq$bc,enriched$bc,pacbio$bc))

union_bc <- data.frame("bc"=sort(union(miseq$bc,union(enriched$bc,pacbio$bc))))
merged <- union_bc %>% dplyr::left_join(miseq, by=c("bc"="bc"))
merged <- merged %>% dplyr::left_join(enriched, by=c("bc"="bc"))
merged <- merged %>% dplyr::left_join(pacbio, by=c("bc"="bc"))
colnames(merged) <- c("bc","car.miseq", "read_count.miseq","car.enriched", "read_count.enriched","car.pacbio", "read_count.pacbio")

##filter enriched data supported by only 1 read exist
#nrow(merged[which(merged$read_count.enriched==1),])
#enriched_filter <- which(merged$read_count.enriched==1)
#merged$car.enriched[enriched_filter] <- NA
#merged$read_count.enriched[enriched_filter] <- NA

merged$consensus <- apply(merged %>% dplyr::select(car.miseq, car.enriched, car.pacbio),1,function(x) names(sort(table(x),decreasing=T))[1])
merged$consensus_count <- apply(merged %>% dplyr::select(car.miseq, car.enriched, car.pacbio),1,function(x) sort(table(x),decreasing=T)[1])
merged$consensus2nd <- apply(merged %>% dplyr::select(car.miseq, car.enriched, car.pacbio),1,function(x) names(sort(table(x),decreasing=T))[2])
merged$consensus2nd_count <- apply(merged %>% dplyr::select(car.miseq, car.enriched, car.pacbio),1,function(x) sort(table(x),decreasing=T)[2])
...
merged$consensus_read_count[which(merged$car.enriched==merged$consensus)] <- merged$consensus_read_count[which(merged$car.enriched==merged$consensus)] + merged$read_count.enriched[which(merged$car.enriched==merged$consensus)]
...
merged$consensus2nd_read_count[which(merged$car.enriched==merged$consensus2nd)] <- merged$consensus2nd_read_count[which(merged$car.enriched==merged$consensus2nd)] + merged$read_count.enriched[which(merged$car.enriched==merged$consensus2nd)]
...
merged$consensus_final[which(!is.na(merged$car.miseq) & !is.na(merged$car.enriched) & !is.na(merged$car.pacbio) & merged$consensus_count==1 & merged$consensus2nd_count==1)] <- NA
...
merged$consensus_labels <- apply(merged %>% dplyr::select(car.miseq, car.enriched, car.pacbio,consensus_final),1,function(x) paste(unlist(gsub("car\\.","", c(names(x)[which(as.character(x)==as.character(x)[4])]))),collapse = "&"))
...
##filter only 1 read and only exist in enriched data
## enriched_filter <- which(merged$consensus_read_count_final==1 & merged$consensus_labels == "enriched")
## merged$consensus_final[enriched_filter] <- NA
...
#head(merged[which(!is.na(merged$car.miseq) & !is.na(merged$car.enriched) & !is.na(merged$car.pacbio) & merged$consensus_count==1 & merged$consensus2nd_count==1),])
